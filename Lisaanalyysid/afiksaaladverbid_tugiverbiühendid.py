...
valjund_fail = "afiksaaladverbid_tulemused.csv"
...
with open(csv_fail, encoding="utf-8") as f:
    for rida in csv.DictReader(f, delimiter=";"):
        ...
        for token in lause.tokens:
            if token.head == verbi_id and token.deprel == "compound:prt":
                leiud.append((rida["Lause_ID"], token.lemma, verb.lemma))

with open(valjund_fail, "w", encoding="utf-8", newline="") as f:
    writer = csv.writer(f, delimiter=";")
    writer.writerow(["Lause_ID", "Afiksaaladverb", "Verb"])
